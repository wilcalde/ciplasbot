...
COLS = {
    ...
    "causa": ["Causa_Paro", "Causa", "Motivo", "Causa de Paro"] # Nueva columna
}
...
def _read_efficiency_history_from_sqlite(end_date: date, months: int = 4) -> pd.DataFrame:
    ...
    try:
        ...
        query = f"SELECT {', '.join(select_list)} FROM \"{table}\" WHERE area = 'fileteado'"
...
# --- NUEVO: Generación de Gráfica de Pareto ---

def _generate_pareto_tp(df: pd.DataFrame, start_str: str) -> str:
    ...
    # Seleccionamos causas hasta llegar al 80% (incluyendo la que rompe el 80)
    ...
    bars = plt.barh(plot_df[c_causa], plot_df[c_tp], color='skyblue', edgecolor='navy')
    plt.xlabel('Horas Perdidas')
    plt.title(f'Causas del 80% del Tiempo Perdido - Gasa\nTotal Horas: {total_tp:.2f}h')
    ...
    plt.tight_layout()
    chart_path = os.path.join(REPORTS_DIR, f"pareto_tp_gasa_{start_str}.png")
...
class ReporteGasa(FPDF):
    ...
    def set_cell_color(self, value):
        if value < 85: self.set_text_color(200, 0, 0)
    ...
    def draw_final_summary(self, totals):
        ...
        gap_objetivo = 85.0 - prod_total
        msg = (f"El proceso estuvo por debajo del objetivo de productividad (85%) en {gap_objetivo:.2f}% "
    ...
    def draw_pareto_section(self, img_path):
        ...
        self.multi_cell(0, 5, "Nota: La grafica muestra las causas que acumulan el 80% de las horas de paro registradas. Enfocarse en estas causas permitira la mayor mejora en productividad.")
...
def build_pdf_gasa(df_range_filtered: pd.DataFrame, start: date, end: date) -> tuple[str, list[str]]:
    ...
    mask = _starts_with_ci(df_range_filtered[c_art], "CAG") & _starts_with_ci(df_range_filtered[c_maq], "FILET")
    ...
    # Generar Pareto
    chart_path = _generate_pareto_tp(df_filt, start.isoformat())
...
def handle_gasa_message(_pk, _txt): return False

__all__ = ["COLS", "build_pdf_gasa", "handle_gasa_message"]