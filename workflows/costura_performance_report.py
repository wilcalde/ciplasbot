# workflows/costura_performance_report.py
# Genera el informe de desempeño sección Costura + Tabla Referencias (Incluye Tarifa)
# + Gráfico de Máquinas y Unidades (Sin desperdicio)
# lectura por qr operarios planta.
...
# ARCHIVOS / RUTAS
# =========================
OPERATORS_FILE = os.path.join(CONFIG_DIR, "operators_costura.json")
REPORTS_DIR = os.path.join(CONFIG_DIR, "costura_reports")
os.makedirs(REPORTS_DIR, exist_ok=True)

PERFORMANCE_REQUESTS_DB = os.path.join(CONFIG_DIR, "costura_requests_log.jsonl")
TZ = "America/Bogota"

# =========================
# ORÍGENES DE DATOS (COSTURA)
# =========================
...
BASE_COLS = {
    ...
    "tarifa": ["Tarifa", "tarifa", "TARIFA", "Valor", "valor"] # <-- NUEVA COLUMNA AGREGADA
}
...
def _clean_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    """Unifica la columna Maquina para que todo esté en mayúsculas sin espacios extra."""
    if df is None or df.empty: return df
    maq_col = _find_col(df, BASE_COLS["maq"])
    if maq_col:
        df[maq_col] = df[maq_col].astype(str).str.strip().str.upper()
        # En caso de que queden valores "NAN" como string
        df[maq_col] = df[maq_col].replace("NAN", "SIN MAQUINA")
    return df
...
def _download_data() -> pd.DataFrame | None:
    try:
        ...
        print(f"❌ Error descargando base principal (Costura): {e}")
        return None
...
def _compute_metrics_for_operator(df: pd.DataFrame) -> dict:
    metrics = {"fecha_min": "N/D", "fecha_max": "N/D", "unidades_totales": 0}
    ...
    if uni_col:
        metrics["unidades_totales"] = pd.to_numeric(df[uni_col], errors="coerce").fillna(0).sum()
...
def _aggregate_downtime_by_cause_for_month(df: pd.DataFrame, month: int, year: int) -> pd.DataFrame:
    ...
    s_fecha = _safe_to_datetime(df[fecha_col])
    if s_fecha.dropna().empty: return df.iloc[0:0].copy()
...
def _aggregate_references_for_month(df: pd.DataFrame, month: int, year: int) -> pd.DataFrame:
    ...
    tp_col = _find_col(df, BASE_COLS["tp"])
    cs_col = _find_col(df, BASE_COLS["cs"])
    tarifa_col = _find_col(df, BASE_COLS["tarifa"]) # Extracción de tarifa
    
    if not fecha_col or not ref_col or not uni_col: return pd.DataFrame()
    ...
    g["unidades_producidas"] = pd.to_numeric(g["unidades_producidas"], errors="coerce").fillna(0)
    g = g.sort_values("unidades_producidas", ascending=False, kind="stable").reset_index(drop=True)
    
    cols_to_return = ["referencia", "unidades_producidas", "prod_pct", "tarifa"]
...
def _plot_daily_bars_with_productivity_units(daily_prod: pd.DataFrame, pivot_mach: pd.DataFrame, machines: list, operator_name: str, month: int, year: int) -> str | None:
    ...
    img_name = f"grafico_diario_unidades_costura_{_slug(operator_name)}_{year}{month:02d}.png"
...
def _plot_bar_downtime_by_cause(gcause: pd.DataFrame, operator_name: str, month: int, year: int) -> str | None:
    ...
    fig.tight_layout()
    img_name = f"grafico_causas_costura_{_slug(operator_name)}_{year}{month:02d}.png"
    img_path = os.path.join(REPORTS_DIR, img_name)
    fig.savefig(img_path, bbox_inches="tight")
    plt.close(fig)
    return img_path
...
def _plot_changeovers_by_day_bar(daily: pd.DataFrame, operator_name: str, month: int, year: int) -> str | None:
    ...
    fig.tight_layout()
    img_name = f"grafico_cambios_costura_{_slug(operator_name)}_{year}{month:02d}.png"
    img_path = os.path.join(REPORTS_DIR, img_name)
    fig.savefig(img_path, bbox_inches="tight")
    plt.close(fig)
    return img_path
...
def _generate_ai_analysis(
    ...
    kpis: dict, pareto_df: pd.DataFrame,
    pivot_mach: pd.DataFrame
) -> str:
    ...
    system_prompt = (
        "Eres CiplasBot, analista de planta. Escribe para OPERARIOS de COSTURA: lenguaje sencillo, motivador y directo. "
    ...
    user_prompt = (
        ...
        "Genera los 4 puntos de análisis solicitados sin mencionar desperdicio."
    )
...
def _build_pdf(
    ...
    references_df: pd.DataFrame, ai_text: str | None
) -> str:
    
    date_str = datetime.now().strftime("%Y-%m-%d")
    fname = f"Informe_Costura_{_slug(operator_name)}_{date_str}.pdf"
    ...
    pdf.cell(page_w / 2, 8, "Área: Costura", align="R", ln=1)
    ...
    # 2. Rango de Fecha y Totales
    pdf.set_text_color(0, 0, 0)
    pdf.set_font("Helvetica", "", 11)
    pdf.cell(0, 6, _sanitize_pdf_text(f"Rango del informe: {metrics.get('fecha_min','N/D')} a {metrics.get('fecha_max','N/D')}"), ln=1)
    pdf.set_font("Helvetica", "B", 11)
    pdf.cell(0, 8, _sanitize_pdf_text(f"Total Unidades Producidas: {int(metrics.get('unidades_totales', 0)):,}"), ln=1)
    ...
    # 4. Causas de Tiempo Perdido (Barras)
    ...
    # 6. Tabla de Referencias Procesadas (Actualizado con Tarifa)
    if references_df is not None and not references_df.empty:
        ...
        col_w_ref = page_w * 0.45
        col_w_uni = page_w * 0.20
        col_w_prod = page_w * 0.20
        col_w_tarifa = page_w * 0.15
        
        pdf.cell(col_w_ref, 8, "Referencia", border=1, align="C", fill=True)
        pdf.cell(col_w_uni, 8, "Unidades", border=1, align="C", fill=True)
        pdf.cell(col_w_prod, 8, "% Productividad", border=1, align="C", fill=True)
        pdf.cell(col_w_tarifa, 8, "Tarifa", border=1, align="C", fill=True, ln=1)
        
        pdf.set_font("Helvetica", "", 9)
        for _, row in references_df.iterrows():
            ref_name = str(row.get("referencia", "N/D"))
            if len(ref_name) > 38: ref_name = ref_name[:35] + "..."
            
            uni_val = int(row.get("unidades_producidas", 0))
            prod_val = row.get("prod_pct")
            tarifa_val = row.get("tarifa")
            
            try:
                tarifa_str = f"${float(tarifa_val):,.0f}" if pd.notna(tarifa_val) else "N/D"
            except (ValueError, TypeError):
                tarifa_str = "N/D"
            
            pdf.cell(col_w_ref, 6, _sanitize_pdf_text(ref_name), border=1, align="L")
            pdf.cell(col_w_uni, 6, f"{uni_val:,}", border=1, align="R")
            
            if pd.notna(prod_val):
                pdf.cell(col_w_prod, 6, f"{prod_val:.1f}%", border=1, align="R")
            else:
                pdf.cell(col_w_prod, 6, "N/D", border=1, align="R")
                
            pdf.cell(col_w_tarifa, 6, tarifa_str, border=1, align="R", ln=1)

    # 7. Análisis de desempeño IA
    if ai_text:
...
# FLUJO PRINCIPAL
# =========================
def handle_costura_performance_report_request(inbound_phone_raw: str, original_text: str) -> bool:
    msg_norm = (original_text or "").strip().lower()
    
    # Trigger para Costura
    if msg_norm not in ("informe costura", "reporte costura"):
        return False
    ...
    if not op:
        send_whatsapp_message(to_norm, "⚠️ No encuentro tu número en la base de operarios de Costura.")
        _log_request(inbound_phone_raw, "", "requested", {"ok": False, "reason": "not_in_costura"})
        return True
    ...
    # ==========================================
    # NUEVO: INTERCEPTOR MACRO-GERENCIAL (WILSON CALDERON)
    # ==========================================
    if area == "GERENTE_PLANTA":
        from workflows.manager_planta_report import handle_manager_planta_report
        return handle_manager_planta_report(inbound_phone_raw, operator_name, to_norm)
    # ==========================================

    if area != "COSTURA":
        send_whatsapp_message(to_norm, f"⚠️ Este comando es exclusivo para Costura. Tu área es: *{area}*.")
    ...
    try:
        send_whatsapp_message(to_norm, f"🧵 Hola *{friendly}*, generando tu Informe de Costura... ⏳")
    except: pass
    ...
    ai_text = _generate_ai_analysis(operator_name, month, year, kpis, g_cause, pivot_mach)
    ...
    pdf_path = _build_pdf(
        ...
        references_df, ai_text
    )

    # 6. Envío y Limpieza
    sent_ok = False
    try:
        send_whatsapp_document(to_norm, pdf_path, caption="📄 Aquí tienes tu Informe de Costura 🧵")
        sent_ok = True
    except Exception as e:
    ...
    try:
        artifacts = [chart_daily_units, chart_bar_cause, chart_changes]
