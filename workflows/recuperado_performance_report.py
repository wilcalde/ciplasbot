# workflows/recuperado_performance_report.py
# Genera el informe de desempeño sección Recuperado + Desperdicio + Tabla Referencias
...
# ARCHIVOS / RUTAS
# =========================
OPERATORS_FILE = os.path.join(CONFIG_DIR, "operators_recuperado.json")
REPORTS_DIR = os.path.join(CONFIG_DIR, "recuperado_reports")
os.makedirs(REPORTS_DIR, exist_ok=True)

PERFORMANCE_REQUESTS_DB = os.path.join(CONFIG_DIR, "recuperado_requests_log.jsonl")
TZ = "America/Bogota"

# =========================
# ORÍGENES DE DATOS (RECUPERADO)
# =========================
SHEET_XLSX_URL = "https://docs.google.com/spreadsheets/d/1dCtQnMpb3oVAjl3QYe-xF0mk7WWMUpGrviYrj7LJ3Pg/export?format=xlsx"
...
def _download_data() -> pd.DataFrame | None:
    try:
        ...
        print(f"❌ Error descargando base principal (Recuperado): {e}")
        return None
...
def _generate_ai_analysis(
    ...
    system_prompt = (
        "Eres CiplasBot, analista de planta. Escribe para OPERARIOS DE RECUPERADO: lenguaje sencillo, motivador y directo. "
...
def _build_pdf(
    ...
    fname = f"Informe_Recuperado_{_slug(operator_name)}_{date_str}.pdf"
    ...
    pdf.cell(page_w / 2, 8, "Área: Recuperado", align="R", ln=1)
...
# FLUJO PRINCIPAL
# =========================
def handle_recuperado_report_request(inbound_phone_raw: str, original_text: str) -> bool:
    msg_norm = (original_text or "").strip().lower()
    
    if msg_norm not in ("informe recuperado", "reporte recuperado"):
        return False
    ...
    if not op:
        send_whatsapp_message(to_norm, "⚠️ No encuentro tu número en la base de operarios de Recuperado. Pide a sistemas que te registren en *operators_recuperado.json*.")
        _log_request(inbound_phone_raw, "", "requested", {"ok": False, "reason": "not_in_recuperado"})
        return True

    area = op.get("area", "").strip().upper()
    if area != "RECUPERADO":
        send_whatsapp_message(to_norm, f"⚠️ Este comando es exclusivo para Recuperado. Tu área es: *{area}*.")
        _log_request(inbound_phone_raw, op.get("Apellidos_Nombres", ""), "requested", {"ok": False, "reason": "wrong_area"})
        return True
    ...
    try:
        send_whatsapp_message(to_norm, f"♻️ Hola *{friendly}*, generando tu Informe Recuperado... ⏳")
    except: pass
    ...
    sent_ok = False
    try:
        send_whatsapp_document(to_norm, pdf_path, caption="📄 Aquí tienes tu Informe de Desempeño Recuperado ♻️")
        sent_ok = True
    except Exception as e:
