...
# ────────────────────────────────────────────────────────────────────────────────
# PDF principal PLANAS
# ────────────────────────────────────────────────────────────────────────────────
def build_pdf_planas(df_range_filtered: pd.DataFrame, start: date, end: date) -> tuple[str, list[str]]:
    """
    Construye el PDF de PLANAS (FIPLA*) con el mismo layout de GASA/LENO.
    Retorna: (pdf_path, [temp_image_paths])
    """
    # Indicadores y tablas
    units = _compute_units_by_lines_planas(df_range_filtered)   # ← FIPLA*
    prod_planas = _compute_productivity_filete_planas(df_range_filtered)
    causes_df   = _downtime_causes_filete_planas(df_range_filtered)
    refs_df     = _table_by_reference_planas(df_range_filtered)
    turno_df    = _turno_prod_and_puestos_planas(df_range_filtered)
    oper_df     = _table_operario_total_planas(df_range_filtered)

    gauge_path, gauge_ar = _plot_gauge_percent(prod_planas, "Productividad Filete Planas")
    bars_path,  bars_ar  = _plot_downtime_bars_horizontal_planas(causes_df)
    turno_path, turno_ar = _plot_turno_bars_and_puestos_planas(turno_df)
    ...
    pdf.cell(0, 8, _sanitize_pdf_text("Filete Planas"), ln=1)
    pdf.set_font("Helvetica", "", 11)
    pdf.cell(0, 6, _sanitize_pdf_text(f"Productividad del periodo: {_fmt_pct(prod_planas)}"), ln=1)
    ...
    used_h_table = _render_table_refs_planas(pdf, refs_df, pdf.l_margin, y1, left_w, max_rows=16)
    used_h_chart = 0.0
    ...
    y2 = pdf.get_y()
    _render_table_operario_total_planas(pdf, oper_df, pdf.l_margin, y2, page_w)
    ...
    # Salida
    fname = f"Analisis_Fileteado_PLANAS_{start.isoformat()}_{end.isoformat()}.pdf"
    out_path = os.path.join(REPORTS_DIR, fname)
    pdf.output(out_path)
