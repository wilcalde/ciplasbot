...
def _find_costura_supervisors() -> list[tuple[str, str]]:
    supervisors: list[tuple[str, str]] = []
    users_data = {}
    try:
        with open(SUPERVISORS_FILE, encoding="utf-8") as f:
    ...
    for user in _collect_user_pools(users_data):
        ...
        area = (user.get("area") or user.get("linea") or user.get("linea_produccion") or "").casefold()
        process = (user.get("process") or "").casefold()
        if "costura" in role or "costura" in area or "costura" in process:
            _push_supervisor(user.get("name") or user.get("nombre"), user.get("phone_e164") or user.get("phone"))
    ...
    try:
        ...
        SUPERVISORS = {}
    for name, phone in SUPERVISORS.items():
        if "costura" in str(name).casefold():
            _push_supervisor(str(name), phone)
...
def handle_fileteado_efficiency_request(phone_key: str, text: str) -> bool:
    ...
    if low == "informe eficiencia":
        ...
        send_whatsapp_message(
            phone_key,
            ("📌 *Selecciona el informe de eficiencia*:\n"
             "1) Fileteado\n"
             "2) Costura\n\n"
    ...
    if state.get("awaiting_menu"):
        choice = low.strip()
        if choice in ("1", "fileteado", "fileteado area", "fileteado línea", "fileteado linea"):
        ...
        if choice in ("2", "costura", "seccion costura", "sección costura"):
            try:
                from workflows.costura_report import _download_costura_df
                df = _download_costura_df()
            except Exception as e:
                send_whatsapp_message(phone_key, f"❌ No pude leer la base de costura: {e}")
                sessions[phone_key].pop("fileteado_eff_state", None)
                return True
            if df is None or df.empty:
            ...
            dmin, dmax, fecha_col = _date_range_in_df(df)
            if not dmin or not dmax:
                send_whatsapp_message(phone_key, "ℹ️ No encontré una columna de fecha válida en la base de costura.")
                sessions[phone_key].pop("fileteado_eff_state", None)
                return True
            sessions.setdefault(phone_key, {})["fileteado_eff_state"] = {
                "awaiting_range": True,
        ...
        send_whatsapp_message(
            phone_key,
            "⚠️ Opción no reconocida. Responde con *1* (Fileteado) o *2* (Costura)."
        )
        return True
    ...
    if state.get("awaiting_range"):
        ...
        target = state.get("target") or "fileteado"
        if target == "costura":
            try:
                from workflows.costura_report import _download_costura_df
                df = _download_costura_df()
        ...
        if df is None or df.empty:
            send_whatsapp_message(phone_key, "❌ No pude leer la base solicitada en este momento.")
            sessions[phone_key].pop("fileteado_eff_state", None)
            return True
        ...
        if target == "costura":
            supervisors = _find_costura_supervisors()
            supervisor_name = supervisors[0][0] if supervisors else "supervisor de costura"
        else:
        ...
        if target == "costura":
            _notify_recipients(recipients_admin, "📄 Generando informe de eficiencia de costura...")
        else:
        ...
        if target == "costura":
            try:
                ...
                costura_pdf, costura_tmp = build_pdf_costura(df_sel, start, end)
                temp_paths.extend([costura_pdf, *(costura_tmp or [])])
                _send_reports_to_recipients(recipients_admin, costura_pdf, "📄 Informe eficiencia – COSTURA")
            except Exception as e:
                send_whatsapp_message(phone_key, f"❌ Error generando informe COSTURA: {e}")
        else:
            try:
    ...
    if state.get("awaiting_confirm"):
        reply = low.strip()
        target = state.get("target") or "fileteado"
        supervisor_name = state.get("supervisor_name") or (
            "supervisor de costura" if target == "costura" else "supervisor de fileteado"
        )
        ...
        if reply in ("si", "sí", "s", "yes"):
            if supervisor_phones:
                ...
                followup = (
                    ...
                    f"{state.get('rango_texto', 'Rango de fecha del informe')} del area de "
                    f"{'costura' if target == 'costura' else 'fileteado'}. "
