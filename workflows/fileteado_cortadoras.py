...
def _generate_pareto_tp_cortadoras(df: pd.DataFrame, start_str: str) -> str:
    ...
    bars = plt.barh(plot_df[c_causa], plot_df[c_tp], color='#fc8d62', edgecolor='#b15928')
    plt.xlabel('Horas Perdidas')
    plt.title(f'Causas del 80% del Tiempo Perdido - Cortadoras\nTotal Horas: {total_tp:.2f}h')
    ...
    plt.tight_layout()
    chart_path = os.path.join(REPORTS_DIR, f"pareto_tp_cortadoras_{start_str}.png")
...
class ReporteCortadoras(FPDF):
    ...
    def draw_pareto_section(self, img_path):
        ...
        self.multi_cell(0, 5, "Nota: El grafico resalta las causas que concentran el 80% del tiempo perdido en las Cortadoras. La optimizacion de estos puntos criticos permitira mejorar significativamente el indicador de productividad.")
...
def build_pdf_cortadoras(df_range_filtered: pd.DataFrame, start: date, end: date) -> tuple[str, list[str]]:
    ...
    df_filt = df_range_filtered.loc[_starts_with_ci(df_range_filtered[c_maq], "CORT")].copy()
    ...
    # Generar Pareto para Cortadoras
    chart_path = _generate_pareto_tp_cortadoras(df_filt, start.isoformat())
    ...
    pdf = ReporteCortadoras(start, end)
    pdf.add_page()
...
def handle_cortadoras_message(_pk, _txt): return False

__all__ = ["COLS", "build_pdf_cortadoras", "handle_cortadoras_message"]